    ...
    tweet && Object.keys(tweet).length > 0 ? (
      <div className="tweet-wrapper">
        ...
        <div className="tweet-body-wrapper">
          ...
          <div className="tweet-date">2:06 AM · Jun 12, 2020</div>
        ...
        {tweet.replies.length > 0 ? (
          tweet.replies.map((replie) => (
            <div key={replie._id} className="tweet-replies-wrapper">
              ...
              <div className="reply-tweet-body">
                <div>
                  <span className="tweet-user-name">{replie.user.name}</span>
                  <span className="reply-header-username">
                    @{replie.user.username}
                  </span>
                  <span className="reply-header-dot">·</span>
                  <span className="reply-header-date">1h</span>
                </div>
                <div>
                ...
                <div className="reply-tweet-content">{replie.body}</div>
                <div className="reply-tweet-interactions">
                  <div className="reply-int-icon" onClick={toggleModal}>
                    ...
                    <div
                      onClick={() => favTweet(replie._id)}
                      className="card-icon-value"
                    >
                      {replie.replies.length === 0 ? "" : replie.replies.length}
                    </div>
                  </div>
                  <div className="reply-int-icon retweet-int">
                    ...
                    <div className="card-icon-value">
                      {replie.retweets.length === 0
                        ? ""
                        : replie.retweets.length}
                    </div>
                  </div>
                  <div className="reply-int-icon heart-int">
                    <LikeButton tweetId={replie._id} />
                    <div className="card-icon-value">
                      {replie.likes.length === 0 ? "" : replie.likes.length}
                    </div>
                  </div>
